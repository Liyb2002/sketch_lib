...
PROMPT = (
    "You are performing STRICT masked image editing on a sketch drawing.\n\n"

    "You are given TWO images of the same size:\n"
    "1) The original input sketch image.\n"
    "2) A binary mask image.\n\n"

    "Mask semantics (CRITICAL):\n"
    "- White pixels (≈255): the ONLY pixels you are allowed to change.\n"
    "- Black pixels (≈0): MUST be copied from the input image EXACTLY, pixel-for-pixel.\n\n"

    "Your output image MUST satisfy:\n"
    "- Outside the white mask, the output image is STIRCTLY IDENTICAL to the input image. NOTHING should be changed\n"
    "- Inside the white mask, you may modify pixels to improve the boundary quality.\n\n"

    "What the white region contains:\n"
    "- A narrow band along the boundary where two sketch pieces meet.\n"
    "- Possible broken strokes, tiny gaps, overlaps, misaligned lines, or jagged edges.\n\n"
    ...
    "- Preserve the original hand-drawn sketch style.\n\n"

    "ABSOLUTE rules:\n"
    "- Do NOT modify any pixel outside the white mask.\n"
    "- Do NOT move, scale, rotate, or reshape any part of the sketch.\n"
    "- Do NOT change the overall layout or geometry.\n"
    "- Do NOT add new objects or new features.\n\n"

    "If a pixel is outside the white mask, it must appear in the output exactly as it appears in the input image.\n\n"

    "Output:\n"
    "- Return exactly one image, same resolution as the input.\n"
)
...
def main():
    ...
    for view_dir in view_dirs:
        ...
        mask_png = os.path.join(fix_dir, "sum_after_mask_boundary_out.png")
        ...
        for attempt in range(3):
            try:
                ...
                raw_out_path = os.path.join(fix_dir, "gemini_original_output.png")
                write_bytes(raw_out_path, raw_bytes)

                # existing masked composite
